...
import 'package:kids_learning_tool/Lessons/Verb/verb_search_bar.dart';
...
class _VerbState extends State<Verb> {
  ...
  late List<VerbItem> verbs;
  List<VerbItem> assignToStudent = [];
  int _index = 0;
  late int len;
  ...
  Future loadAudio() async {
    await _audioPlayer.setAudioSource(
        AudioSource.uri(Uri.file(verbs[_index].audio)),
          ...
          actions: [
            IconButton(
                onPressed: () async {
                  ...
                  setState(() {
                    _index = max(0,
                        verbs.indexWhere((element) => element.text == result));
                  });
  ...
  Widget verbCardWidget() {
    VerbItem verb = verbs.elementAt(_index);
    List<String> images = verb.getImgList();
              ...
              child: Column(
                ...
                children: <Widget>[
                  Row(
                    ...
                    children: <Widget>[
                      Checkbox(
                          value: verb.isSelected,
                          onChanged: (value) {
                            setState(() {
                              verb.isSelected = !verb.isSelected;
                              if (verb.isSelected) {
                                assignToStudent.add(verbs[_index]);
                              } else {
                                assignToStudent.remove(verbs[_index]);
                              }
                            });
                          }),
                      IconButton(
